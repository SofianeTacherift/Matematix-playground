...
#define LIST_NAME(T) T##_array_list
...
#define ARRAY_LIST(T, TYPE_ALIAS) \
    ...
    static LIST_NAME(TYPE_ALIAS)* new_##TYPE_ALIAS##_array_list() { \
        LIST_NAME(TYPE_ALIAS) *ARRAY_LIST = malloc(sizeof(LIST_NAME(TYPE_ALIAS))); \
    ...
    \
    \
    static void pop_##TYPE_ALIAS(LIST_NAME(TYPE_ALIAS)* list) { \
    ...
    \
    static bool TYPE_ALIAS##_array_list_empty(LIST_NAME(TYPE_ALIAS)* list) {\
        return list->size==0;\
    }\
    \
    static T *last_##TYPE_ALIAS##_from(LIST_NAME(TYPE_ALIAS) *list) { \
        if (list->size<=0) {\
            return NULL; \
        }\
